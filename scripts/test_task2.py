...
def test_full_flow():
    print('=' * 70)
    print('TEST COMPLETO: Tarea 1 (Abrir app + proyecto) + Tarea 2 (Cola AUTO + Boleta)')
    print('=' * 70)
    ...
    try:
        ...
        # Verificar que el título de boleta tiene formato fecha_id
        boleta_title = info.get('boleta_title', '')
        import re
        fecha_id_pattern = r'\d{2}-\d{2}-\d{4}_\d+'
        if re.search(fecha_id_pattern, boleta_title):
            print(f'\n  Titulo de boleta tiene formato correcto (fecha_id): {boleta_title}')
        elif boleta_title:
            print(f'\n  Titulo de boleta no coincide con patron fecha_id: {boleta_title}')
